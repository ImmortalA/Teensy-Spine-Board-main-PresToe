...
uint8_t can_data_b0[MAX_SLOTS];
uint8_t can_command_b0[MAX_SLOTS];
uint8_t can_data_b1[MAX_SLOTS];
uint8_t can_command_b1[MAX_SLOTS];
...
static void setupOneCan(CanT &can, void (*handler)(const CAN_message_t &)) {
    can.begin();
...
void receiveUDPPacket() {
    ...
    while ((size = udp.parsePacket()) > 0) {
        ...
        if (size == 2 && data[0] == RESET_COMMAND) {
            ...
            reset();
        } else if (!waiting_for_daisy_fb && size == TEENSY_UDP_PAYLOAD_BYTES + 1) {
